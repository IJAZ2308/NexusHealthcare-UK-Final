...
class _AdminDoctorApprovalScreenState extends State<AdminDoctorApprovalScreen>
  ...
  void initState() {
    ...
    _listenDoctorsRealtime();
  }

  void _listenDoctorsRealtime() {
  ...
  Future<void> _sendNotification(String doctorId, String status) async {
    try {
      final url =
          'https://YOUR_CLOUD_FUNCTION_URL/notifyDoctor'; // Replace with your deployed Cloud Function URL
      await http.post(
        Uri.parse(url),
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text("What would you like to do with this doctor?"),
          actions: [
            TextButton(
              onPressed: () {
